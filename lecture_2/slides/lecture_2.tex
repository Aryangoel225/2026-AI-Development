...
\begin{itemize}
	...
	\begin{enumerate}
		\item \texttt{load\_resumes()} - loads all the resumes into a dictionary
		\item \texttt{analyze\_resume()} - runs a prompt against a resume with structured output
	\end{enumerate}
	\item There are two job reqs in the directory also: \texttt{job\_req\_senior.md} and \texttt{job\_req\_junior.md}
